...
const EditPractice: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPractice = async () => {
      try {
        const response = await Api.get(`/practices/${id}`, auth.data.token);
        const { data, statusCode } = response;
        if (statusCode === 200) {
          setPractice(data.microsite);
        } else {
          Swal.fire({
            title: "Error",
            text: `${data.message}`,
  ...
  const initialValues = {
    ...
    payment_expiration_time: practice.payment_expiration_time || "",
  ...
  const handleSubmit = async (values: any) => {
    try {
      const response = await Api.patch(`/practices/${id}`, values, auth.data.token);
      const { data, statusCode } = response;
      if (statusCode === 200) {
        Swal.fire({
          title: "Success",
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Edit Practice</h1>
