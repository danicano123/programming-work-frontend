...
const PracticeStrategyList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPracticeStrategy = async () => {
      try {
        const { data, statusCode } = await Api.get("/practice-strategys/active");
        if (statusCode === 200) {
  ...
  const handleNavigate = (slug: string, practiceStrategyId: string) => {
    navigate(`/practice-strategys/${slug}`, { state: { practiceStrategyId } });
  };

  const handleNavigateToform = (slug: string, practiceStrategyId: string) => {
    navigate(`/practice-strategys/${slug}/form/${practiceStrategyId}`, {
