...
const CreateProgramPe: React.FC = () => {
  ...
  const handleSubmit = async (values: any) => {
    setIsLoading(true);
    try {
      const response = await Api.post("/programPe", values, auth.data.token);
      const { data, statusCode } = response;
      if (statusCode === 201) {
        Swal.fire({ 
          title: "Success",
          text: "Programa pe creado con exito",
          icon: "success",
        });
        navigate("/program-pe-dashboard");
      } else {
        Swal.fire({
          title: "Error",
          text: `${data.message}`,
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Crear Programa Pe</h1>
...
export default CreateProgramPe;
