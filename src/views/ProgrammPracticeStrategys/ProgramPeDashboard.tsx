  ...
  const handleToggleIsActive = async (
    ...
    try {
      const response = await Api.patch(
        `/programm-practice-strategys/${programPeId}/is-active`,
        {
          is_active: !isActive,
        },
        auth.data.token
      ...
      if (statusCode === 200) {
        const updatedprogrammPracticeStrategy = programmPracticeStrategy.map((programmPracticeStrategy) =>
          programmPracticeStrategy.id === programmPracticeStrategy.Id
            ? { ...programmPracticeStrategy, is_active: !isActive }
            : programmPracticeStrategy
        );
        setProgrammPracticeStrategy(updatedprogrammPracticeStrategy);
        Swal.fire({
          title: "Success",
          text: "programmPracticeStrategy actualizado con exito",
          icon: "success",
        });
      } else {
        Swal.fire({
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold mb-4">Tablero de Programa Practica Estrategia</h1>
        <button
          onClick={() => navigate("/create-programm-practice-strategy")}
          className="bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded"
        >
          Crear Programa Practica Estrategia
        </button>
      </div>
          ...
          {programmPracticeStrategy.map((programmPracticeStrategy) => (
            <tr key={programmPracticeStrategy.id}>
              <td className="py-2 px-4 border-b text-center">
                {programmPracticeStrategy.practicestrategy}
              </td>
              <td className="py-2 px-4 border-b text-center">
                {programmPracticeStrategy.program}
              </td>
              <td className="py-2 px-4 border-b text-center space-x-4">
               <button
                  onClick={() =>
                    navigate(`/read-programm-practice-strategy/${programmPracticeStrategy.id}`)
                  }
                ...
                <button
                  onClick={() =>
                    navigate(`/edit-programm-practice-strategy/${programmPracticeStrategy.id}`)
                  }
                  className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                >
                  Editar
