...
const CreateAnProgram: React.FC = () => {
  ...
  const handleSubmit = async (values: any) => {
    setIsLoading(true);
    try {
      const response = await Api.post("/anProgram", values, auth.data.token);
      const { data, statusCode } = response;
      if (statusCode === 201) {
        Swal.fire({ 
          title: "Success",
          text: "An Programa creado con exito",
          icon: "success",
        });
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Crear An Programa</h1>
...
export default CreateAnProgram;
